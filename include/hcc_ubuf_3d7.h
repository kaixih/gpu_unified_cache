#ifndef _HCC_UBUF_3D7
#define _HCC_UBUF_3D7
...
{
public:
    virtual T load(int idx, int idy, int idz)restrict(amp)=0;
    virtual void store(T v, int idx, int idy, int idz)restrict(amp)=0;
    virtual void glb2buf(array<T> &in, int type=NONE)restrict(amp)=0;
};
...
public:
    int gidx, gidy, gidz;
public:
    T load(int idx, int idy, int idz) restrict(amp)
    {
        return ACC_3D((*l1m), idx, idy, idz);
    }
    void store(T v, int idx, int idy, int idz) restrict(amp)
    {
        ACC_3D((*l1m), idx, idy, idz) = v;
    }
    L1Buffer(int k, int j, int i, int h, tiled_index<3> &tidx) restrict(amp): z(k), m(j), n(i), halo(h)
    {
        gidx = tidx.global[2] + halo;
        gidy = tidx.global[1] + halo;
        gidz = tidx.global[0] + halo;
    }
    void glb2buf(array<T> &in, int type=NONE) restrict(amp)
    {
        l1m = &in;
    }
    // in hcc, this works
...
template<class T>
T Sten_L1M_Fetch(L1Buffer<T> buf, int pz, int py, int px, int tc=0) restrict(amp)
{
...
public:
    ...
    LDSBuffer(int k, int j, int i, int h, tiled_index<3> &t) restrict(amp): z(k), m(j), n(i), halo(h)
    {
        tile_static T sm[10*6*10];
        local = sm;
    ...
    void glb2buf(array<T> &in, int type=NONE) restrict(amp)
    {
        if(type == BRANCH)
        {
            ...
            if(lidy == 8   ) LOC_3D(lidz  ,lidy+1,lidx  ) = ACC_3D(in,gidz  ,gidy+1,gidx  );
            if(lidz == halo) LOC_3D(lidz-1,lidy  ,lidx  ) = ACC_3D(in,gidz-1,gidy  ,gidx  );
            if(lidz == 4   ) LOC_3D(lidz+1,lidy  ,lidx  ) = ACC_3D(in,gidz+1,gidy  ,gidx  );
        } else if(type == CYCLIC)
        {
...
template<class T>
T Sten_LDS_Fetch(LDSBuffer<T> buf, int pz, int py, int px, int tc=0) restrict(amp)
{
