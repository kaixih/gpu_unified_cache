...
// static int ghst_layers = 3;
// static int warp_size = 32;
// static int sten_dim = 1;

static int warp_dim[3] = {3, 3, 0};
static int crs_fct = 4;
...
void print_shfl_code(vector<vector<vector<vector<intPair>>>> &friend_ids, int job_id, int id0, int id1, int id2)
{
    cout << "tx" << job_id << " = __shfl(reg" << friend_ids[job_id][id2][id1][id0].second;
    ...
    while(count0 < warp_size)
    {
        ...
        if(lb <= tmp_rw && tmp_rw < rb)
        {
            if(reg_id_flag)
            {
                cout << "t" << buf[buf_id] << job_id << " = __shfl(reg" << reg_id;
                cout << ", friend_id" << job_id << ");" << endl;
                mark[buf_id] = count0;
                buf_id++;
                reg_id_flag = false;
    ...
    {
        cout << "return (lane_id < " << mark[0] << " )? tx";
        cout << job_id << ": ((lane_id < " << mark[1] << ")? ty" << job_id << ": tz" << job_id << ");" << endl;
    } else if(mark[0] != -1 )
    {
        cout << "return (lane_id < " << mark[0] << " )? tx";
        cout << job_id << ": ty" << job_id << ";" << endl;
    } else 
    {
        cout << "return tx" << job_id << ";" << endl;
    }
