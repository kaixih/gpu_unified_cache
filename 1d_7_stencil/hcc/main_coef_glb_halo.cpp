...
#pragma omp parallel for
    ...
    {
        OUT_1D(i) = ARG_1D(0,i)*IN_1D(i-3) + 
                    ARG_1D(1,i)*IN_1D(i-2) + 
                    ARG_1D(2,i)*IN_1D(i-1) + 
                    ARG_1D(3,i)*IN_1D(i  ) + 
                    ARG_1D(4,i)*IN_1D(i+1) + 
                    ARG_1D(5,i)*IN_1D(i+2) + 
                    ARG_1D(6,i)*IN_1D(i+3) ;
    }
}
        ...
        int n, int halo)
{
    ...
    completion_future fut = parallel_for_each(comp_tile, [=, &in, &out, &args](tiled_index<1> tidx) restrict(amp) {
        ...
        tx0 = __shfl(reg0, friend_id0);
        ty0 = __shfl(reg1, friend_id0);
        sum0 += ARG_1D(3,gid)*((lane_id < 61)? tx0: ty0);

        friend_id0 = (lane_id+4 )&(warpSize-1);
        tx0 = __shfl(reg0, friend_id0);
        ty0 = __shfl(reg1, friend_id0);
        sum0 += ARG_1D(4,gid)*((lane_id < 60)? tx0: ty0);

        friend_id0 = (lane_id+5 )&(warpSize-1);
        tx0 = __shfl(reg0, friend_id0);
        ty0 = __shfl(reg1, friend_id0);
        sum0 += ARG_1D(5,gid)*((lane_id < 59)? tx0: ty0);

        friend_id0 = (lane_id+6 )&(warpSize-1);
        tx0 = __shfl(reg0, friend_id0);
        ty0 = __shfl(reg1, friend_id0);
        sum0 += ARG_1D(6,gid)*((lane_id < 58)? tx0: ty0);
        ...
        int n, int halo)
{
    ...
    completion_future fut = parallel_for_each(comp_tile, [=, &in, &out, &args](tiled_index<1> tidx) restrict(amp) {
        ...
        ty1 = __shfl(reg2, friend_id0);
        sum0 += ARG_1D(3,gid   )*((lane_id < 61)? tx0: ty0);
        sum1 += ARG_1D(3,gid+64)*((lane_id < 61)? tx1: ty1);
        ...
        ty1 = __shfl(reg2, friend_id0);
        sum0 += ARG_1D(4,gid   )*((lane_id < 60)? tx0: ty0);
        sum1 += ARG_1D(4,gid+64)*((lane_id < 60)? tx1: ty1);
        ...
        ty1 = __shfl(reg2, friend_id0);
        sum0 += ARG_1D(5,gid   )*((lane_id < 59)? tx0: ty0);
        sum1 += ARG_1D(5,gid+64)*((lane_id < 59)? tx1: ty1);
        ...
        ty1 = __shfl(reg2, friend_id0);
        sum0 += ARG_1D(6,gid   )*((lane_id < 58)? tx0: ty0);
        sum1 += ARG_1D(6,gid+64)*((lane_id < 58)? tx1: ty1);
        ...
        int n, int halo)
{
    ...
    completion_future fut = parallel_for_each(comp_tile, [=, &in, &out, &args](tiled_index<1> tidx) restrict(amp) {
        ...
        sum0 += ARG_1D(3,gid    )*((lane_id < 61)? tx0: ty0);
        sum1 += ARG_1D(3,gid+64 )*((lane_id < 61)? tx1: ty1);
        sum2 += ARG_1D(3,gid+128)*((lane_id < 61)? tx2: ty2);
        sum3 += ARG_1D(3,gid+192)*((lane_id < 61)? tx3: ty3);
        ...
        sum0 += ARG_1D(4,gid    )*((lane_id < 60)? tx0: ty0);
        sum1 += ARG_1D(4,gid+64 )*((lane_id < 60)? tx1: ty1);
        sum2 += ARG_1D(4,gid+128)*((lane_id < 60)? tx2: ty2);
        sum3 += ARG_1D(4,gid+192)*((lane_id < 60)? tx3: ty3);
        ...
        sum0 += ARG_1D(5,gid    )*((lane_id < 59)? tx0: ty0);
        sum1 += ARG_1D(5,gid+64 )*((lane_id < 59)? tx1: ty1);
        sum2 += ARG_1D(5,gid+128)*((lane_id < 59)? tx2: ty2);
        sum3 += ARG_1D(5,gid+192)*((lane_id < 59)? tx3: ty3);
        ...
        sum0 += ARG_1D(6,gid    )*((lane_id < 58)? tx0: ty0);
        sum1 += ARG_1D(6,gid+64 )*((lane_id < 58)? tx1: ty1);
        sum2 += ARG_1D(6,gid+128)*((lane_id < 58)? tx2: ty2);
        sum3 += ARG_1D(6,gid+192)*((lane_id < 58)? tx3: ty3);
...
int main(int argc, char **argv)
{
...
#ifdef __DEBUG
    Init_Args_1D(args, 7, n, halo, 1.0);
#else
    Init_Args_1D(args, 7, n, halo, 0.14);
#endif
    ...
    printf("Throughput   : %.3f (GB/s)\n", GetThroughput(ARGC_1D7, 1, 1, n+2*halo, ITER, time_wo_pci, sizeof(DATA_TYPE)));
