    ...
    for(int k = 0; k < Z; k++)
    {
        ...
        for(int j = 0; j < M; j++)
        {
            ...
            for(int i = 0; i < N; i++)
            {
                int w = (i == 0)      ? i : i - 1;
                int e = (i == N-1)    ? i : i + 1;
                OUT_3D(k,j,i) = ARG_3D(0,k,j,i) * IN_3D(b  ,j  ,i  ) +
                                ARG_3D(1,k,j,i) * IN_3D(k  ,n  ,i  ) +
                                ARG_3D(2,k,j,i) * IN_3D(k  ,j  ,w  ) +
                                ARG_3D(3,k,j,i) * IN_3D(k  ,j  ,i  ) +
                                ARG_3D(4,k,j,i) * IN_3D(k  ,j  ,e  ) +
                                ARG_3D(5,k,j,i) * IN_3D(k  ,s  ,i  ) +
                                ARG_3D(6,k,j,i) * IN_3D(t  ,j  ,i  ) ;
            }
        }
    }
}
...
int main(int argc, char **argv)
{
...
#ifdef __DEBUG
    Init_Args_3D(args, 7, z, m, n, 1.0);
#else
    Init_Args_3D(args, 7, z, m, n, 0.14);
#endif
    ...
    printf("Throughput   : %.3f (GB/s)\n", GetThroughput(ARGC_3D7, z, m, n, ITER, time_wo_pci, sizeof(DATA_TYPE)));
