        ...
        int M, int N) 
{
    ...
    int w = (i == 0)      ? i : i - 1;
    int e = (i == N-1)    ? i : i + 1;
    OUT_2D(j,i) = ARG_2D(0,j,i) * IN_2D(n,w) +
                  ARG_2D(1,j,i) * IN_2D(j,w) +
                  ARG_2D(2,j,i) * IN_2D(s,w) +
                  ARG_2D(3,j,i) * IN_2D(n,i) +
                  ARG_2D(4,j,i) * IN_2D(j,i) +
                  ARG_2D(5,j,i) * IN_2D(s,i) +
                  ARG_2D(6,j,i) * IN_2D(n,e) +
                  ARG_2D(7,j,i) * IN_2D(j,e) +
                  ARG_2D(8,j,i) * IN_2D(s,e) ;
}
...
__global__ void Stencil_Cuda_Shfl4_2(DATA_TYPE *in, DATA_TYPE *out, DATA_TYPE *args,
        int M, int N) 
{
    unsigned int tid = threadIdx.x + threadIdx.y * blockDim.x;
    unsigned int lane_id = tid % warpSize;
...
__global__ void Stencil_Cuda_Shfl8_2(DATA_TYPE *in, DATA_TYPE *out, DATA_TYPE *args,
        int M, int N) 
{
    unsigned int tid = threadIdx.x + threadIdx.y * blockDim.x;
    unsigned int lane_id = tid % warpSize;
...
int main(int argc, char **argv)
{
...
#ifdef __DEBUG
    Init_Args_2D(args, 9, m, n, 1.0);
#else
    Init_Args_2D(args, 9, m, n, 0.11);
#endif
    ...
    printf("Throughput   : %.3f (GB/s)\n", GetThroughput(ARGC_2D9, 1, m, n, ITER, time_wo_pci, sizeof(DATA_TYPE)));
    ...
    for(int i=0; i< ITER; i++)
    {
        Stencil_Cuda_SmX<<<dimGrid1, dimBlock1>>>(in_d, out_d, 
                args_d,
                m, n); 
        swap(in_d, out_d);
    }
